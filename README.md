...
- [打开 2026-07-19 本地端到端演示（MP4）](demo-output/evidence-rag-mvp-self-recorded-20260719-email-masked.mp4)
...
由 1 位评审者声明式逐题复核的 72 条 FastAPI 官方文档题集已完成正式检索评测：在 `top_k=3`、3 条预热的固定本机 Docker 环境下，RRF-only 的 Recall@3 为 `0.847`、MRR 为 `0.743`；启用 Reranker 后 Recall@3 保持 `0.847`、MRR 提升到 `0.819`，平均检索耗时由 `62.7 ms` 增至 `1390.0 ms`。完整的题集/审核/配置哈希与报告见 [docs/verification.md](docs/verification.md)。这些数字只反映文件级检索命中与排序，不能替代答案、引用、拒答、端到端延迟或成本结论，也不是独立第三方或多评审结论。

低置信度守卫已在固定的 72 条支持问题和 24 条开发者声明式越界问题上离线校准，Reranker 顶部得分阈值为 `0.02`：保留支持问题 `65/72 (90.3%)`，拒绝越界问题 `22/24 (91.7%)`。代价是同一批次文件级 Recall@5 从阈值前 `64/72 (88.9%)` 降到阈值后 `59/72 (81.9%)`。越界题不是独立第三方审核，因此该结果是工程校准证据，不是新的公开质量指标。

旧 UUID 引用协议的固定 72 题批次已由 1 位评审者完成声明式逐题审核：49 条回答、21 条引用守卫拒答、2 条上游失败；答案 `47/49 (95.9%)`、引用 `45/49 (91.8%)`、拒答 `0/21 (0.0%)`。这份审核满足当前 MVP 对人工答案、引用和拒答指标记录的要求，但不应夸大为独立第三方或多评审结论。短引用键修复后另有一份现存可审计批次：56 条回答、16 条引用守卫拒答、无上游失败；72/72 次模型完成调用均有 token 与成本快照，按 2026-07-15 记录的 DeepSeek 缓存未命中价格保守估算总成本为 `0.127286 CNY`、平均每次模型完成 `0.00176786 CNY`。新批次尚未完成人工审核，因此这些 outcome 只能说明引用协议改善信号，不能写成新的答案、引用或拒答通过率；只有需要发布优化后的新质量指标时，才需使用 `/review` 重新逐题复核。
...
演示操作路径和录屏后的最小验证清单见 [docs/demo-script.md](docs/demo-script.md)。
