...
- 已快照的单次成本，以及采用短引用键后的新一轮真实模型批次复测
...
同一 72 题已完成一次真实证据问答批次：49 条回答带服务端校验引用，21 条因模型引用不合法被证据守卫拒答，2 条上游调用失败；成功回答的模型耗时均值为 `4090.7 ms`、P95 为 `7546 ms`，共报告 `79481` tokens。该批次已由 1 位评审者逐题完成声明式人工审核：答案 `47/49 (95.9%)`、引用 `45/49 (91.8%)`、拒答 `0/21 (0.0%)`。这反映的是旧 UUID 引用协议下的固定批次；短引用键修复尚未进行新的真实模型复测，成本也尚无该批次的单价快照，因此不应把它表述成最终优化后的效果或单次成本结论。
