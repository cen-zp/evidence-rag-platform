...
- 独立题集上的人工答案正确率、引用正确率、拒答恰当性，以及已快照的单次成本
...
同一 72 题已完成一次真实证据问答批次：49 条回答带服务端校验引用，21 条因模型引用不合法被证据守卫拒答，2 条上游调用失败；成功回答的模型耗时均值为 `4090.7 ms`、P95 为 `7546 ms`，共报告 `79481` tokens。批次结果和待填写审核表已保存，但尚未逐题人工判定答案、引用和拒答质量，也未据此形成成本结论；因此它不是准确率或简历指标。
