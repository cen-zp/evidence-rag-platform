...
当前已实现本地单用户的文件上传、Markdown/PDF 解析、分块和 Qdrant 写入。Qdrant 向量目前由本地哈希向量器生成，只用于验证索引工程链路，并不代表语义检索效果。`User`、`Conversation`、`Message`、检索、引用与评测会在真实产品流程需要时再加入，避免一次性建立尚未验证的数据结构。
