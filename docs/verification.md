...
- 回归与运行：不含 `.env` 的隔离副本全量测试 64/64、Ruff、前端 ESLint/production build 均通过；API/Worker/Web 已用新镜像重建，迁移 `20260715_0009` 已应用，API 健康且 OpenAPI 已暴露计时、浏览器回写与汇总接口。本轮校准未调用 DeepSeek。
