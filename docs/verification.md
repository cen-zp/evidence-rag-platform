...
- 实现验证：低分候选会在模型调用前过滤，空结果走 `retrieval-guard`；RRF-only 对照不套用不同量纲的阈值。后端响应新增模型、检索和服务端管线耗时，前端测量浏览器实际端到端耗时；三类耗时会绑定到助手消息，并由知识库级接口汇总样本量、均值和 P95。
- 回归与运行：不含 `.env` 的隔离副本全量测试 65/65、Ruff、前端 ESLint/production build 均通过；API/Worker/Web 已用新镜像重建，迁移 `20260715_0009` 已应用，API 健康且 OpenAPI 已暴露计时、浏览器回写与汇总接口。本轮校准未调用 DeepSeek。
- 真实浏览器守卫样本：登录后的本地工作台提交开发者声明式越界题 `unsupported-01`（离线顶部 Reranker 分数 `0.00023448`，低于阈值 `0.02`）。修复后请求返回 `retrieval-guard`、模型耗时 `0 ms`、0 条引用；检索 `1461 ms`、服务端全链路 `26666 ms`、浏览器端到端 `26687 ms`。模型调用汇总在请求前后均为 264 次，证明该样本未调用 DeepSeek。
- 缺陷与样本边界：第一次真实请求在结果持久化后触发可选字段序列化错误，未写回浏览器耗时；`cd8d91c` 修复并新增守卫 SSE 回归测试。数据库因此保留 2 条守卫消息，只有修复后的 1 条具有浏览器端到端值。该请求包含容器重建后的本地模型冷启动，只证明计时、拒答与持久化链路，不代表稳态均值或 P95。
- 真实浏览器回答样本：经用户明确授权，仅提交 1 条固定题 `errors-07`（离线顶部 Reranker 分数 `0.9992938`，现存短键批次曾成功回答）。结果使用 `deepseek-v4-flash`，模型 `3581 ms`、检索 `1480 ms`、服务端全链路 `5073 ms`、浏览器端到端 `5092 ms`，供应商返回 `1267` total tokens；服务端校验 1 条 `06-handling-errors.md` 片段后才展示回答。模型调用与 usage 覆盖均从 264 增至 265，证明只发生 1 次新模型完成调用。
- 成本边界：该单次浏览器调用没有新的单价快照，已估算成本调用数仍为 215，因此不事后补算或冒充账单；正式平均成本仍使用上方绑定当日缓存未命中价格快照的 72 次短键批次。当前延迟汇总包含 3 条消息（1 回答、2 守卫），其中只有修复后的回答和守卫各 1 条具有浏览器端到端值；小样本均值和 P95 不作为稳定性能结论。

结论：低置信度规则已从“无命中才拒答”提升为有固定报告约束的 Reranker 分数守卫；正常回答与守卫拒答各 1 条真实浏览器完整请求均已记录。计时链路验收完成，但样本量不足以宣称稳定延迟分布；演示视频仍待完成。
