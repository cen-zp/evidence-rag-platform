...
`fastapi-official-cases.jsonl` 是基于公开 FastAPI 官方文档建立的 72 条中文 AI 协助题目。其问题、参考答案和预期来源已按 `fastapi-official-review.csv` 完成声明式独立人工逐题复核，因此可进入正式**检索**评测；这不等于模型答案已经人工判分。`fastapi-official-formal-answer-review.csv` 是一次真实模型批次生成的待填写答案审核表，在它被人工完成并通过校验前，不能从该批次得出答案、引用或拒答通过率。
