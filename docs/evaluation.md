...
以下离线命令复用生产分块、BGE、BM25、RRF 与 Reranker，不连接数据库且不调用语言模型：

```bash
cd apps/api
uv run python -m app.evaluation.confidence \
  --corpus-dir ../../evals/corpora/fastapi-official-2026-07-14 \
  --supported-cases ../../evals/independent/fastapi-official-cases.jsonl \
  --unsupported-cases ../../evals/calibration/fastapi-official-unsupported-cases.jsonl \
  --output ../../evals/results/fastapi-official-confidence-calibration.json \
  --top-k 5 \
  --candidate-count 10 \
  --minimum-supported-acceptance 0.9 \
  --threshold-step 0.01
```

报告保存逐题顶部 Reranker 分数、命中文件、选择约束、阈值前后 Recall，以及语料和两组案例的 SHA-256。72 条支持题沿用原有单人声明式审核；24 条越界题由开发者从语料明确未覆盖的主题中声明式编写，没有独立第三方审核。因此阈值可用于当前工程拒答守卫，但不能被描述成独立测试集上的最终质量指标。
...
旧 UUID 批次的人工审核已经完成并通过完整性校验，不需要重复操作。本页保留给将来需要发布短引用键优化后质量指标时使用：本地 Docker Web 启动后，打开 `http://localhost:3000/review`，或从工作台顶部进入“72 题真人审核”。页面只加载 batch ID `02dbf511…`、报告 SHA-256 `0050e4ed…55fef` 的短引用键固定批次；服务端会同时校验 batch ID、哈希与 72 题覆盖，任一不符即停止加载以避免串批。回答题分别点答案/引用通过或不通过，守卫拒答题只点拒答合理或不当；进度和备注仅保存在当前浏览器的批次专属本地草稿中，不会把判断发送给模型或写进后端数据库。
...
`fastapi-official-cases.jsonl` 是基于公开 FastAPI 官方文档建立的 72 条中文 AI 协助题目。其问题、参考答案和预期来源已按 `fastapi-official-review.csv` 由 1 位评审者完成声明式逐题复核，因此可进入正式**检索**评测。固定真实模型批次的答案审核已保存在 `fastapi-official-formal-answer-review-human.csv`，并通过完整性校验；两份审核均不应夸大为独立第三方或多评审结论。
